...
class game:
    ...
    def name(self):
        input_box = pygame.Rect(WIDTH // 2 -300 , HEIGHT // 2 -200, 200, 50)
...
def load_last_score(user_name):
    comm = """SELECT score, level FROM snake
             WHERE user_name = %s
             ORDER BY played_at DESC
             LIMIT 1;"""
    try:
        with conn.cursor() as cur:
            cur.execute(comm, (user_name,))
            result = cur.fetchone()
            if result:
                return result
...
def user(user_name, score, level):
    comm = """INSERT INTO snake(user_name, score, level)
             VALUES(%s, %s, %s);"""
    try:
        with conn.cursor() as cur:
            cur.execute(comm, (user_name, score, level))
            conn.commit()
...
user(snake.player_name, score, level)
pygame.quit()
conn.close()